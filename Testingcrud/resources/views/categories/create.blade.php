    ...
    <div class="card-body">
       ...
       @endif
        <form action="{{ isset($category) ? route('categories.update',$category->id) : route('categories.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <input type="file" class="form-control" id="image" name="image" value="{{ isset($category) ? $category->image : '' }}">
              </div>

            <button type="submit" class="btn btn-dark mt-2">{{ isset($category) ? 'Edit' : 'Create' }} </button>
          </form>
    </div>
</div>
@endsection
